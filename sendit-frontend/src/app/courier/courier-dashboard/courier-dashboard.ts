    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Show coordinates first, then get address
          this.location = `Lat: ${this.courierCoords.lat.toFixed(6)}, Lng: ${this.courierCoords.lng.toFixed(6)} (Accuracy: ${position.coords.accuracy}m)`;
          
          this.reverseGeocode(this.courierCoords.lat, this.courierCoords.lng);
          
          // Update courier location in backend
          if (this.courier) {
            this.courierService.updateCourierLocation(this.courier.id, {
              locationLat: this.courierCoords.lat,
              locationLng: this.courierCoords.lng,
      ...
      this.messageService.showError('Geolocation is not supported by this browser.');
      this.setRandomLocation();
    }
  }
